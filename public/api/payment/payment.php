...
$lockFile = __DIR__ . '/lockfile.txt';

// Попытка создать файл блокировки
$lockHandle = fopen($lockFile, 'w');

if ($lockHandle === false || !flock($lockHandle, LOCK_EX | LOCK_NB)) {
    // Файл блокировки уже существует, запрос уже выполняется
    echo 'Запрос уже выполняется. Пожалуйста, подождите.';
    exit;
}
...
if ($apiUserData && isset($apiUserData['balance'])) {
    ...
    if ($userB['userWithdraw'] >= 4 && $apiBalance >= $userB['userWithdraw']) {
        ...
        if (isset($_SESSION['last_request_time']) && (time() - $_SESSION['last_request_time']) < $minTimeBetweenRequests) {
            ...
            // Добавьте здесь код для обработки успешного запроса на вывод средств
        }
